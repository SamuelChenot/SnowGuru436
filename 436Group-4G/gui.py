...
class StartPage(tk.Frame):
    def __init__(self, master):
        ...
        tk.Button(self, text="See All Mountains",bg="gray45", image = self.photos[0],
                  command=lambda: master.switch_frame(AllMountains)).pack()
        tk.Button(self, text="Nearby Mountains",bg="gray45", image = self.photos[1],
                  command=lambda: master.switch_frame(NearbyMountains)).pack()
        tk.Button(self, text="Saved Mountains",bg="gray45", image = self.photos[2],
                  command=lambda: master.switch_frame(SavedMountains)).pack()
        tk.Button(self, text="Hourly Weather",bg="gray45", image = self.photos[3],
...
class AllMountains(tk.Frame):
    def __init__(self, master):
        ...
        #Add button pics from C:/But_Pics/
        self.photos = []
        for i in range(1, 70):
